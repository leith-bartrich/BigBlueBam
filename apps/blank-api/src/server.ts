...
fastify.setErrorHandler((error: FastifyError, request, reply) => {
  ...
  const isAppError = error.name === 'BlankError' || (error as any).code;
  const message =
    statusCode >= 500
...
await fastify.register(formRoutes, { prefix: '/v1' });
await fastify.register(fieldRoutes, { prefix: '/v1' });
await fastify.register(submissionRoutes, { prefix: '/v1' });
...
try {
  ...
  fastify.log.info(`Blank API listening on ${env.HOST}:${env.PORT}`);
} catch (err) {
