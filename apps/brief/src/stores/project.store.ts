...
interface ProjectState {
  ...
  setProjects: (projects: BriefProject[]) => void;
}

// Persist the active project selection to localStorage
function loadActiveProjectId(): string | null {
  try {
    return localStorage.getItem('brief_active_project_id') || null;
  } catch {
    return null;
  }
}
...
function saveActiveProjectId(id: string | null): void {
  try {
    if (id) {
      localStorage.setItem('brief_active_project_id', id);
    } else {
      localStorage.removeItem('brief_active_project_id');
    }
  } catch {
    // ignore
  }
