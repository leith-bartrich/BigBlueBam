...
test.describe('Brief — UI-API Agreement', () => {
  test('document list in UI matches API response', async ({ page, screenshots, context, request }) => {
    const cookies = await context.cookies();
    const csrf = readCsrfTokenFromCookies(cookies);
    const api = new DirectApiClient(request, '/brief/api', csrf || undefined);
    const checker = new UiApiChecker(page, api);

    await page.goto('/brief/documents');
  ...
  test('templates list in UI matches API response', async ({ page, screenshots, context, request }) => {
    const cookies = await context.cookies();
    const csrf = readCsrfTokenFromCookies(cookies);
    const api = new DirectApiClient(request, '/brief/api', csrf || undefined);
    const checker = new UiApiChecker(page, api);

    await page.goto('/brief/templates');
    await page.waitForTimeout(2000);
  ...
  test('API health endpoint returns expected structure', async ({ request }) => {
    const api = new DirectApiClient(request, '/brief/api');
