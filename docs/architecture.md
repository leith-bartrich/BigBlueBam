...
|       +-- bigbluebam/
|
|-- scripts/                  Utility scripts; dev pipeline lives in scripts/dev/
|-- docker-compose.yml        Production stack (data services + 17 app services + migrate one-shot)
|-- docker-compose.override.yml   gitignored, generated by scripts/dev/compose-overrides.sh for per-service hot-reload
