...
import { Shield, Search, LogIn, Loader2, Users, FolderKanban, ListChecks, TicketIcon, MessageSquare, Building2, UserPlus, TrendingUp, ArrowLeft, Settings, Mail, Download, Globe, Bot, ArrowRight, Plus, Pencil, Trash2, LayoutGrid, PhoneCall } from 'lucide-react';
...
// ─── Calling credentials (SuperUser) ────────────────────────────────────────

function CallingCredentialsCard() {
  // Read-only card showing the current LiveKit credential state. The
  // actual generated values live in the operator's .env (and the
  // rendered infra/livekit/livekit.yaml). This card is informational —
  // when the credentials need to rotate, the operator re-runs the
  // deploy script, which regenerates and re-renders.
  const { data, isLoading } = useQuery<{ data: { using_devkey: boolean; livekit_url: string | null; providers: { stt: string | null; llm: string | null; tts: string | null } } }>({
    queryKey: ['superuser', 'calling-credentials'],
    queryFn: () => api.get('/superuser/calling-credentials'),
  });

  const usingDevkey = data?.data?.using_devkey ?? true;
    ...
    <section className="rounded-lg border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 p-6">
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1">
          ...
          <p className="mt-1 text-sm text-zinc-500 dark:text-zinc-400">
            LiveKit powers voice and video calls in Banter (and, eventually, every
            app in the suite). Credentials are generated by the deploy script and
            written to <code className="text-xs">.env</code> + the rendered
            <code className="text-xs ml-1">infra/livekit/livekit.yaml</code>.
          </p>
          {isLoading ? (
            ...
            <div className="mt-4 space-y-3 text-sm">
              ...
              {usingDevkey && (
                <p className="text-xs text-amber-700 dark:text-amber-300">
                  ⚠ Dev mode uses the published <code>devkey:secret</code> pair from
                  the LiveKit yaml template — fine for local testing, never for
                  production. Re-run <code>node scripts/deploy/main.mjs</code> to
                  generate proper credentials.
                </p>
              )}
              {data?.data?.livekit_url && (
                <div className="text-xs text-zinc-500">
                  Public LiveKit URL: <code>{data.data.livekit_url}</code>
                </div>
              )}
              <div className="rounded-md border border-zinc-200 dark:border-zinc-800 p-3">
                <div className="text-xs font-medium text-zinc-700 dark:text-zinc-300 mb-2">Voice agent providers</div>
                <ProviderRow label="STT" value={data?.data?.providers?.stt} />
                <ProviderRow label="LLM" value={data?.data?.providers?.llm} />
                <ProviderRow label="TTS" value={data?.data?.providers?.tts} />
                <p className="mt-2 text-[11px] text-zinc-500 dark:text-zinc-400">
                  STT, LLM, and TTS providers are read by the voice-agent service.
                  Set them via env vars on first deploy or per-org via the AI
                  Providers settings page (<code>/b3/settings → AI Providers</code>).
                  Without them, the voice agent runs in degraded "log-only" mode
                  and human-only voice/video calls still work.
                </p>
              </div>
            </div>
          )}
