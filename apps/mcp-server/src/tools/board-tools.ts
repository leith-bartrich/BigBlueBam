...
export function registerBoardTools(server: McpServer, api: ApiClient, boardApiUrl: string): void {
  ...
  server.tool(
    'board_create',
    'Create a new visual collaboration board. `template_id` accepts either a UUID or a template name (case-insensitive).',
    {
  ...
  server.tool(
    'board_update',
    'Update board metadata. Provide only the fields to change. `id` accepts either a UUID or a board name.',
    {
  ...
  server.tool(
    'board_archive',
    'Archive a board (soft delete). `id` accepts either a UUID or a board name.',
    {
      id: z.string().describe('Board UUID or name'),
    },
    async ({ id }) => {
  ...
  server.tool(
    'board_read_elements',
    'Read all elements on a board. Returns structured data with positions, text, and types. `id` accepts either a UUID or a board name.',
    {
      id: z.string().describe('Board UUID or name'),
    },
    async ({ id }) => {
  ...
  server.tool(
    'board_add_sticky',
    'Add a sticky note to a board. `board_id` accepts either a UUID or a board name.',
    {
  ...
  server.tool(
    'board_add_text',
    'Add a text element to a board. `board_id` accepts either a UUID or a board name.',
    {
      board_id: z.string().describe('Board UUID or name'),
  ...
  server.tool(
    ...
    'Promote sticky notes to Bam tasks in a project. `board_id` accepts either a UUID or a board name. `project_id` accepts either a UUID or a project name. `phase_id` accepts either a UUID or a phase name (scoped to the resolved project).',
    {
      board_id: z.string().describe('Board UUID or name'),
    ...
    async ({ board_id, project_id, phase_id, ...body }) => {
  ...
  server.tool(
    'board_export',
    'Export a board as SVG or PNG. `id` accepts either a UUID or a board name.',
    {
      id: z.string().describe('Board UUID or name'),
  ...
  server.tool(
    'board_summarize',
    'Get a board summary grouped by frames, including element counts and text content. `id` accepts either a UUID or a board name.',
    {
      id: z.string().describe('Board UUID or name'),
    },
    async ({ id }) => {
