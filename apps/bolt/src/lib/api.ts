...
class ApiClient {
  ...
  constructor(baseUrl = '/bolt/api/v1') {
    this.baseUrl = baseUrl;
  }

  /**
   * Resolve the active org_id so the bolt-api knows which org context to use.
   * We lazily import the auth store to avoid circular dependencies at module init.
   */
  private getOrgId(): string | undefined {
    try {
      const mod = (globalThis as any).__boltAuthStore;
  ...
  private async request<T>(
    ...
    // Send active org context so bolt-api resolves the correct org
