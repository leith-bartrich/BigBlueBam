...
import { BlastSidebar } from '@/components/layout/blast-sidebar';
import { useAuthStore } from '@/stores/auth.store';

type ActiveRoute = { page: string; id?: string };

interface BlastLayoutProps {
  children: ReactNode;
...
function breadcrumbsFor(route: ActiveRoute): Crumb[] {
  switch (route.page) {
    ...
    case 'campaign-new':
      return [{ label: 'Campaigns', href: '/' }, { label: 'New Campaign' }];
    case 'campaign-detail':
      return [{ label: 'Campaigns', href: '/' }, { label: 'Campaign' }];
    case 'templates':
      return [{ label: 'Templates' }];
    case 'template-new':
      return [{ label: 'Templates', href: '/templates' }, { label: 'New Template' }];
    case 'template-edit':
      return [{ label: 'Templates', href: '/templates' }, { label: 'Edit Template' }];
    case 'segments':
      return [{ label: 'Segments' }];
    case 'segment-new':
      return [{ label: 'Segments', href: '/segments' }, { label: 'New Segment' }];
    case 'analytics':
      return [{ label: 'Analytics' }];
    case 'domain-settings':
      return [{ label: 'Settings' }, { label: 'Domains' }];
    case 'smtp-settings':
      return [{ label: 'Settings' }, { label: 'SMTP' }];
    default:
      return [];
  }
}

export function BlastLayout({ children, onNavigate, activeRoute }: BlastLayoutProps) {
    ...
    <div className="flex flex-col h-screen overflow-hidden bg-zinc-50 dark:bg-zinc-950">
      <div className="flex flex-1 overflow-hidden">
        ...
        <aside className="w-[260px] flex-shrink-0 bg-sidebar flex flex-col">
          <BlastSidebar onNavigate={onNavigate} activePage={activeRoute.page} />
        </aside>
        ...
        <div className="flex flex-col flex-1 min-w-0 overflow-hidden">
          ...
          <header className="flex items-center justify-between h-14 px-6 border-b border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 shrink-0">
            ...
            <div className="flex items-center gap-4">
              ...
              />
              <NotificationsBell inAppPrefix="/blast/" onNavigate={onNavigate} />
              <UserMenu user={user} />
            </div>
          </header>
      ...
      <Launchpad isOpen={launchpadOpen} onClose={() => setLaunchpadOpen(false)} currentApp="blast" />
    </div>
  );
}
